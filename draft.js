 ...
 class Employer {
      ...
      return new Promise((resolve, reject) => {
        ...
        const page = 1
        const size = 2
        fetchData("https://api.baserow.io/api/database/rows/table/118042/?filter__field_1007785__link_row_has="+ this.accountDetails.id + "&page="+page+"&size="+size).then(value =>{
  ...
  _candidates(pageNumber, numRows) {
    return new Promise((resolve, reject) => {
      let candidates = [];
      fetchData("https://api.baserow.io/api/database/rows/table/118042/?filter__field_1007785__link_row_has=" + this.accountDetails.id + "&page=" + pageNumber + "&size=" + numRows)
        .then(value => {
          $.each(value.data.results, function (key, candidate) {
            // Process each candidate object and add it to the candidates array
            // ...
            candidates.push(candidate);
          });
          resolve(candidates); // Resolve the promise with the candidates
        })
        .catch(error => {
          reject(error); // Reject the promise if an error occurs
        });
